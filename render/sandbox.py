...
`manimuser`. Network is OFF unless a caller explicitly opts in. This is the day-one
sandbox the PRD asks for; M7 hardens it (resource caps, ephemeral FS).

`build_command` is pure (unit-testable without Docker); `render` executes it.
"""
...
def build_command(
    ...
    *,
    ...
    """Build the `docker run` argv that renders `scene_file::scene_name` in a sandbox.

    `--network=none` unless `network=True` (the sandbox invariant). `work_dir` is mounted
    at /manim; Manim writes its `media/` tree there, so outputs land on the host.
    """
...
def render(
    ...
    *,
    ...
    cmd = build_command(
        ...
        quality=quality, still=still, network=network, image=image,
    )
    proc = subprocess.run(
